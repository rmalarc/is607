...
We first want to know what type of observations are more often found in this population. In order to do this analysis, we need a good control group, preferable composed of precursors or populations that may have a high risk for developing the disease of interest.

The purpose of this project is to build a database of diseases and other related diseases by srapping a well-known clinical literature search engine such as [PubMed](http://www.ncbi.nlm.nih.gov/pubmed).

Let's say D is the set of disease terms such that d is a given disease term. In this case, we will use disease terms found in ICD9 classification data.
...
dxref_data[dxref_data$CCS.DIAGNOSIS.CATEGORIES %in%
             c(".Z  ",".   ",".A  ","254","255","256","257","258","259"),"is_valid"] <-FALSE 
dxref_data[grep("E Codes: ",dxref_data$CCS.DIAGNOSIS.CATEGORIES.LABELS),"is_valid"] <-FALSE 
...
scrape_pubmed_entries <- function(ref_data,entry_type) {
  #ref_data<-dxref_data
  #entry_type<-"absolute_count"
  
  coll<-paste(ns,entry_type,sep=".")
  entries_processed <- NA
  # get previously exported records
  buf <- mongo.bson.buffer.create()
  mongo.bson.buffer.append(buf, "entry_type", entry_type)
  mongo_query <- mongo.bson.from.buffer(buf)
  entries_processed <- mongo.cursor.to.data.frame(mongo.find(m,coll,mongo_query))
...
dxref_data_x_join <- merge(x = absolute_count_entries_processed[absolute_count_entries_processed$is_valid,]
                           , y = absolute_count_entries_processed[absolute_count_entries_processed$is_valid,]
                           , by = NULL)

dxref_data_x_join$CCS.DIAGNOSIS.CATEGORIES.x<-as.character(dxref_data_x_join$CCS.DIAGNOSIS.CATEGORIES.x)
dxref_data_x_join$CCS.DIAGNOSIS.CATEGORIES.y<-as.character(dxref_data_x_join$CCS.DIAGNOSIS.CATEGORIES.y)

#Remove permutations, we just want combinations
dxref_data_x_join <- dxref_data_x_join[dxref_data_x_join$CCS.DIAGNOSIS.CATEGORIES.x
                                       >dxref_data_x_join$CCS.DIAGNOSIS.CATEGORIES.y,]
